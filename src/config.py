"""
Configuration module for UI color scheme constants.

This file stores predefined color values used throughout the application's
user interface to ensure a consistent look and feel.
"""
BG_COLOR = "#2E2E2E"
FG_COLOR_LIGHT = "#E0E0E0"
BUTTON_BG_COLOR = "#4A4A4A"
BUTTON_FG_COLOR = "#FFFFFF"
ACCENT_COLOR = "#007ACC"
STATUS_BG_COLOR = "#3B3B3B"
PREVIEW_BG_COLOR = "#1C1C1C"
ERROR_FG_COLOR = "#FF6B6B"
SUCCESS_FG_COLOR = "#76FF03"
WARNING_FG_COLOR = "#FFD700"
TIMELINE_BG = "#3C3C3C"
TIMELINE_TROUGH = "#5A5A5A"
